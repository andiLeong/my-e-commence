    ...
    <x-form.layout wire:submit.prevent="store">
            ...
            <div>
                ...
                <div class="mt-6 sm:mt-5 space-y-6 sm:space-y-5">
                        ...
                        <x-slot name="slot">
                            <div class="flex items-center">
                                  <span class="h-20 w-20 rounded-full overflow-hidden bg-gray-100">
                                     @if ($cover)
                                          <img class="h-full w-full" src="{{ $cover->temporaryUrl() }}">
                                      @else
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                        <x-form.label class="sm:mt-px sm:pt-2" value="Product photo"/>
                        <div>
                            <x-filepone wire:model="photos" multiple />
                            ...
                            <div class="flex space-x-11">
                                @if ($photos)
                                    @foreach($photos as $photo)
                                        <img class="h-32 w-32 rounded-full " src="{{ $photo->temporaryUrl() }}">
                                    @endforeach
                                @endif
                            </div>
                    </div>
        ...
        <x-slot name="button">
            <div class="flex justify-end">
                ...
                <x-button.icon-on-the-left-btn background="bg-white" hover="hover:bg-gray-50"
                                               class="text-gray-700 border-gray-300 border">
                    Cancel
                ...
                >
                    <x-svg.spin class="hidden text-white" wire:target="store" wire:loading.class.remove="hidden"/>
                    Save
                </x-button.icon-on-the-left-btn>
